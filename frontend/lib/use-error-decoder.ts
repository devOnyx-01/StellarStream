...
// ─── Contract error code → human-readable map ────────────────────────────────
// Numeric codes mirror the Soroban contract's error enum.
// String keys mirror the backend i18n error-dictionary codes.

export interface DecodedError {
  /** Original raw value from the XDR result (code or string key) */
  raw: string | number;
  /** Short human-readable title */
  title: string;
  /** Longer description suitable for support */
  description: string;
  /** Severity level */
...
const CONTRACT_ERROR_MAP: Record<number, Omit<DecodedError, "raw">> = {
  ...
  2: {
    ...
    description: "No stream exists with the provided ID on this contract.",
    severity: "error",
    docsUrl: "https://docs.stellarstream.io/errors/stream-not-found",
  },
  3: {
    title: "Stream Already Exists",
    description: "A stream with this ID has already been created.",
    severity: "error",
  },
  4: {
  ...
  10: {
    ...
    description: "The V1→V2 migration pipeline is temporarily halted.",
    severity: "warning",
  },
  11: {
    title: "Overflow",
  ...
  12: {
    ...
    description: "No streamed funds are available to withdraw at this time.",
    severity: "warning",
  },
  13: {
    title: "Vault Locked",
...
// String-keyed errors (matching backend i18n dictionary + wallet errors)
const STRING_ERROR_MAP: Record<string, Omit<DecodedError, "raw">> = {
  ERR_INVALID_ADDRESS_FORMAT: {
    title: "Invalid Address Format",
    description: "The Stellar address provided is not correctly formatted.",
    severity: "error",
  },
  INVALID_ADDRESS: {
    title: "Invalid Stellar Address",
    description: "The address is not a valid Stellar public key.",
    severity: "error",
  },
  INVALID_SIGNATURE: {
...
const FALLBACK_DECODED: Omit<DecodedError, "raw"> = {
  title: "Unknown Error",
  description:
    "An unrecognised error was returned by the contract. Copy the details below and contact support.",
  severity: "fatal",
};

// ─── XDR result string parser ─────────────────────────────────────────────────
      ...
      (raw: unknown): DecodedError => {
        ...
        const code =
          typeof rawValue === "number"
            ? rawValue
            : extractCodeFromXdr(String(rawValue));
